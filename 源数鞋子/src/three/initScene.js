...
var APP = {
  Player: function(dom) {
    ...
    this.setSize = function(width, height) {
      ...
      this.height = height
      if (camera) {
        let camList = scene.children.filter(item => item.type.indexOf('Camera') !== -1)
        camList.forEach(item => {
          item.aspect = 1920 / 1080
    ...
    function animate() {
      ...
      // if (newcc.changeBloomComposer) {
      //   newcc.changeBloomComposer.render(scene, camera)
      // } else {
    ...
    this.initChange = () => {
      this.change = new newcc.ChangeInterface({
        scene: scene,
        camera: camera,
        controls: controls,
    ...
    loader.load(
      ...
      text => {
        ...
        window.addEventListener('resize', function() {
          ...
          scene.style.width = dom.clientWidth + 'px'
          scene.style.height = dom.clientHeight + 'px'
        })
      },
      function(xhr) {
        //return;
        ...
        let progress = ((xhr.loaded / xhr.total) * 100).toString().substring(0, 4) - 0
