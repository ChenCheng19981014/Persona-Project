...
function Pb(assets) {
  ...
  this.getClickEventObj = (e, accectClickList) => {
    ...
    let intersects = raycaster.intersectObjects(accectClickList.length > 0 ? accectClickList : scene.children, true)
  ...
  this.scalarBoom = (model, scalar) => {
    ...
    model.traverse(function(value) {
      if (value.isMesh) {
        ...
        //计算爆炸方向
        value.worldDir = new THREE.Vector3().subVectors(worldPs, modelCenter).normalize()
        //保存初始坐标
        value.userData.oldPs = new THREE.Vector3(value.position.x, value.position.y, value.position.z)
      }
    })
    model.traverse(function(value) {
  ...
  this.changeModelOpc = (model, opc = 0.5, toOld = false) => {
    ...
    if (model.material.oldOpc === undefined) {
      model.material.oldOpc = model.material.opacity === undefined ? 1 : model.material.opacity
      model.material.oldTransparent = model.material.transparent === undefined ? false : model.material.transparent
    }
    if (toOld) {
   ...
   * @returns 相机动画需要的数据
   */
  this.getCameraAnimaBaseData = (now = false, cx, cy, cz, tx, ty, tz, anima = false) => {
    if (now) {
  ...
  this.pathAnima = (model, animaModel, endCall, isLookAt = false, speed = 0.001) => {
    if (!model.children) throw TypeError('路径动画需要传入的模型children不为空')
...
function effect() {
  ...
  that.addOutLine = needSelectdeList => {
    let outLine = new OutlinePass(new THREE.Vector2(dom.clientWidth, dom.clientHeight), scene, camera)
