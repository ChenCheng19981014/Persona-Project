 ...
 * 修改：
 * 1.添加控制器，controls
 * 2.拦截animation，play中传入自定义的animation
 * 3.添加资源 assets函数，返回出所有的需要的资源
 *   scene: scene,场景
 *   camera: camera,相机
...
var APP = {
  Player: function() {
    ...
    var dom = document.querySelector(".scene");
    ...
    function animate() {
      ...
      //动画更新
      if (controls) controls.update();
      if (scene.mixer && scene.clock)
        scene.mixer.update(scene.clock.getDelta());
