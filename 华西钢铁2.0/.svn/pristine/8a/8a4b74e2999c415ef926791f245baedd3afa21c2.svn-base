    ...
    initScene() {
      ...
      loader.load(
        ...
        function(text) {
          ...
          that.$Bus.$on("initSprite", change.fn.hideSprite);
          that.$Bus.$on("initCamera", change.fn.showCamera);
          that.$Bus.$on("initmonitor", change.fn.monitor);
          that.$Bus.$on("removeCamera", a);
        ...
        function(xhr) {
          ...
          var a = ((xhr.loaded / xhr.total) * 100).toString().substring(0, 4);
          if (!diva) return;
          diva.style.width = a + "%";
          span.innerHTML = a + "%";
          if (a >= 100) {
